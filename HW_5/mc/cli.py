...
def get_parser():
    ...
    home = os.path.join(home, 'Documents', 'NLP', 'Assignment5')
    ...
    parser.add_argument('--out_dir', default=os.path.join(home, 'out'))
    ...
    parser.add_argument('--gru_hidden_size', type=int, default=200)
...
def auto(config, data_type):
    ...
    # do not save and train the model if the current function is val or test
    if data_type == 'train':
...
def main():
    ...
    # configure for 'train' and 'val', and run the model
    if config.train:
        auto(config, 'train')
        val_config = parser.parse_args()
        auto(val_config, 'val')
        val_config.train = False
        # print(config.hidden_size, val_config.hidden_size, config.emb_metadata_path)
        ...
        # configure for the server demo and run
        if config.serve:
            auto(config, 'serve')
            config.fresh = True
            demo.demo(config)
        # configure for the 'test' phase and run
        else:
            auto(config, 'test')
            run.test(config)
