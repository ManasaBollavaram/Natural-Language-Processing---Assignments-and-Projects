...
class SquadData(Data):
    def __init__(self, config, train_data=None):
        ...
        # else load data into the model for training/validation
        else:
            # calling SquadData.load() for loading embeddings matrix, max_context_size, max_question_size
    ...
    def load(self, update=False):
        config = self.config
        # load the train/val SQuAD data into the global config namespace
        with open(config.squad_path, 'r') as fp:
            self.squad = json.load(fp)
        # preprocess the loaded Q&A pairs
        self._prepro(self.squad)
        self._common = self._get_common(self.squad)
        metadata = self._get_metadata(self.squad)
        # get the dataset information in a specified format (startInd, endInd, span, len(x), len(y), idxs etc)
        ...
        # load embeddings matrix, max context and question size
    ...
    # tokenize, strip punctuation of text in data
    def _prepro(self, squad):
        # no need to proceess everything again if it already exists i.e. '--fresh' is not passed
        ...
        for article in tqdm(squad['data'], desc="{} prepro".format(self.config.data_type)):
            ...
            # break if model is in the checking a.k.a draft mode
            if self.config.draft:
                break

    # get common words from the dataset i.e. words occurring with a frequency of more than 10 (default value) and store
    # 2 files, one with idx, other with no idx
    ...
    # gets GloVE word vectors for all possible words in dataset from the given file
    def _get_metadata(self, squad):
        ...
        # uses the module word2vec_dict to load all GloVe word vectors
        ...
        # assign the embeddings matrix to config
    ...
    # loads word vectors, called in _get_metadata
    @property
